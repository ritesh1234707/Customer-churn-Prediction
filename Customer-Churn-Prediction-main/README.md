...
This project solves this by optimizing for **ROC-AUC, Precision, and Recall**, backed by a financial simulation layer. Instead of using a default $0.5$ classification threshold, the pipeline shifts the threshold dynamically to find the exact mathematical sweet spot where retention spend minimizes customer acquisition losses.
...
The simulator sweeps thresholds from $0.0$ to $1.0$ to isolate the operational setting that yields peak profitability.
